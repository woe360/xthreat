...
// //   <div className="relative group">
// //     <motion.div
// //       className="w-full p-8 py-12 rounded-xl bg-[linear-gradient(180deg,#2A2D35_0%,#1F2228_100%)] transition-all duration-300 relative overflow-hidden"
// //       initial={{ "--x": "100%" }}
// //       animate={{ "--x": "-100%" }}
// //       transition={{
// //         repeat: Infinity,
...
//             <div className="relative text-center">
//               <span className="text-6xl font-bold text-black/50">
//                 <span className="text-3xl absolute -left-4 top-0">€</span>
//                 {isYearly ? plan.yearlyPrice : plan.monthlyPrice}
//               </span>
//               <span className="absolute inset-0 text-6xl ml-[1px] font-bold text-neutral-950">
//                 <span className="text-3xl absolute -left-4 top-0">€</span>
...
//             animate={{ opacity: 1, y: 0 }}
//             transition={{ duration: 0.5, delay: 0.4 }}
//             className="mt-14 bg-gray-900/30 [box-shadow:0_-20px_70px_-20px_#ffffff1f_inset] border border-gray-800 rounded-lg p-8 shadow-lg"
//           >
//             <div className="flex flex-col md:flex-row justify-between items-start md:items-stretch space-y-6 md:space-y-0">
//             <div className="w-full md:w-1/2 space-y-6">
//                 <div className="flex flex-col space-y-2 w-full">
...
const PricingCard = ({ plan, isYearly }) => {
  const router = useRouter();
  const handleCTAClick = (link: string, planName: string) => {
    // Remove the existing URL parameters from the link
    const baseLink = link.split('?')[0];
    const searchParams = new URLSearchParams();
    // Ensure planName is lowercase and matches exactly what we expect
  ...
  return (
    <div className="relative group">
      <div className="w-full p-8 py-12 rounded-xl bg-gray-900/30 [box-shadow:0_-20px_70px_-20px_#ffffff1f_inset] border border-gray-800 transition-all duration-300 relative overflow-hidden">
        {/* Content container */}
        <div className="relative z-10">
          {/* Plan name */}
          ...
          <p className="text-gray-400 text-left text-sm min-h-[60px]">
            {plan.description}
          </p>
          <button
            onClick={() => handleCTAClick('/free-trial', plan.name)}
            className="w-full mt-4 py-3 px-4 rounded-lg bg-gray-900/30 [box-shadow:0_-20px_70px_-20px_#ffffff1f_inset] hover:bg-transparent hover:box-shadow-none text-white border border-gray-500 font-medium hover:bg-border-gray-700 transition-all duration-300"
          >
            {plan.cta}
          </button>
        </div>
...
const FeaturesTable = () => {
  const columnClass = "py-6 px-4 md:px-8";
  const features = [
    {
      name: "Free Trial",
      essential: "14-day trial",
  ...
  return (
    <div className="w-full max-w-[1360px] mx-auto overflow-x-auto">
      <div className="min-w-[768px] rounded-xl overflow-hidden bg-gray-900/30 [box-shadow:0_-20px_70px_-20px_#ffffff1f_inset] border border-gray-800">
        <div className="grid grid-cols-4 text-left border-b border-gray-700">
          <div className={`${columnClass} text-sm font-medium text-gray-300`}>Features</div>
          <div className={`${columnClass} text-sm font-medium text-gray-300`}>Essential</div>
          <div className={`${columnClass} text-sm font-medium text-gray-300`}>Core</div>
          <div className={`${columnClass} text-sm font-medium text-gray-300`}>Advanced</div>
        ...
        <div>
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="grid grid-cols-4 text-sm border-b border-gray-700 last:border-0"
            >
              <div className={`${columnClass} text-gray-300`}>{feature.name}</div>
              {[feature.essential, feature.core, feature.advanced].map((value, i) => (
                <div key={i} className={`${columnClass} text-gray-400`}>
...
const PricingPage = () => {
  ...
  const handleCTAClick = (link: string, planName?: string) => {
    if (planName) {
      router.push(`/free-trial?plan=${planName.toLowerCase()}`);
    } else {
      router.push(link);
    }
  };
  ...
  const getPrice = (userCount, planName, isYearly) => {
    const tier = pricingTiers.find(t => userCount <= t.users) || pricingTiers[pricingTiers.length - 1];
    ...
    <div className="min-h-screen font-sans bg-black text-white relative overflow-hidden">
      ...
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8 py-56">
        ...
        >
          <button
            onClick={togglePricing}
            className="bg-gray-900/30 [box-shadow:0_-20px_70px_-20px_#ffffff1f_inset] border border-gray-800 rounded-lg p-1 flex items-center relative"
          >
            <motion.span
              animate={{ x: isYearly ? '100%' : '0%' }}
              transition={{
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5, delay: 0.3 }}
          className="mt-12 max-w-7xl space-y-12 mx-auto px-4 sm:px-0 lg:px-0 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-x-8"
        >
          {pricingPlans.map((plan, index) => (
            <PricingCard key={plan.name} plan={plan} isYearly={isYearly} />
          ))}
        ...
        >
          <div className="bg-gray-900/30 [box-shadow:0_-20px_70px_-20px_#ffffff1f_inset] border border-gray-800 rounded-xl p-12 max-w-7xl mx-auto">
            <h2 className="text-3xl bg-clip-text text-transparent bg-gradient-to-b from-white to-neutral-500 font-semibold mb-4">
              Still not sure?
            </h2>
            <p className="text-gray-400 mb-8 text-lg">
              Try free trial or see how app works!
            </p>
            <div className="space-x-4">
            <Link
              href="/free-trial"
              className="inline-flex items-center px-6 bg-white text-black py-3 border border-white/20 rounded-xl font-medium hover:bg-white/90 transition-colors"
            >
              Free Trial
            </Link>
            <Link
