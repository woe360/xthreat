...
interface MultiChannelPhishingSimulatorProps {
  onComplete?: () => void;
}

const MultiChannelPhishingSimulator: React.FC<MultiChannelPhishingSimulatorProps> = ({ onComplete }) => {
  const [activeChannel, setActiveChannel] = useState<string>('slack');
  const [selectedMessage, setSelectedMessage] = useState<PhishingMessage | null>(null);
  const [identifiedElements, setIdentifiedElements] = useState<string[]>([]);
  const [score, setScore] = useState(0);
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <Shield className="w-8 h-8 text-blue-400" />
              <div>
                <h1 className="text-xl font-medium text-white">Advanced Channel Phishing Simulator</h1>
                <p className="text-sm text-gray-400">Multi-Channel Security Training</p>
              </div>
            </div>
          ...
          {/* Channel Selection */}
          <div className="flex gap-2 p-2 bg-gray-800/30 rounded-lg overflow-x-auto mb-6">
            {['slack', 'teams', 'whatsapp', 'calendar', 'zoom'].map((channel) => (
              <button
                key={channel}
                onClick={() => setActiveChannel(channel)}
          ...
          {/* Message Interface */}
          <div className="bg-gray-800/30 rounded-lg border border-gray-800/40">
            {/* Channel Header */}
            ...
            <div className="p-4 space-y-4">
              {messages
                ...
                .map((message) => (
                  <div key={message.id} className="space-y-4">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-lg bg-gray-800/50 flex items-center justify-center text-blue-400">
                        {message.sender.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="mt-1 p-3 bg-gray-800/50 rounded-lg text-gray-300">
                          {message.content}
                        </div>
                        ...
                        <div className="mt-4 p-4 bg-gray-800/30 rounded-lg border border-gray-800/40">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="font-medium text-white">Message Context</span>
                          </div>
                          <p className="text-sm text-gray-300">{message.context}</p>
                        </div>
                      </div>
