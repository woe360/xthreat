...
interface VendorPortalSimulatorProps {
  onComplete?: () => void;
}

const VendorPortalSimulator: React.FC<VendorPortalSimulatorProps> = ({ onComplete }) => {
  const [currentTab, setCurrentTab] = useState<'notifications' | 'orders' | 'documents'>('notifications');
  const [selectedNotification, setSelectedNotification] = useState<PortalNotification | null>(null);
  const [flaggedNotifications, setFlaggedNotifications] = useState<string[]>([]);
  const [score, setScore] = useState(0);
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <Building2 className="w-8 h-8 text-blue-400" />
              <div>
                <h1 className="text-xl font-medium text-white">Vendor Portal Simulator</h1>
                <p className="text-sm text-gray-400">Supply Chain Security Training</p>
              </div>
            </div>
          ...
          {/* Portal Navigation */}
          <div className="flex gap-2 border-b border-gray-800/40 mb-6">
            {(['notifications', 'orders', 'documents'] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setCurrentTab(tab)}
          ...
          <div className="space-y-4">
            {portalNotifications.map((notification) => (
              <div 
                key={notification.id}
                className="bg-gray-800/30 rounded-lg p-4 border border-gray-800/40"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <div className={`p-2 rounded-lg ${
                      notification.type === 'shipping' ? 'bg-blue-500/10 text-blue-400' :
                      notification.type === 'order' ? 'bg-blue-500/10 text-blue-400' :
                      notification.type === 'invoice' ? 'bg-blue-500/10 text-blue-400' :
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg transition-colors ${
                        flaggedNotifications.includes(notification.id)
                          ...
                          : 'bg-gray-800/50 hover:bg-gray-800/70 text-gray-400'
                      }`}
                    >
                      <AlertTriangle className="w-5 h-5" />
                    </button>
                    <button 
                      className="p-2 rounded-lg bg-gray-800/50 hover:bg-gray-800/70 text-gray-400"
                      onClick={() => setSelectedNotification(notification)}
                    >
                      <Search className="w-5 h-5" />
                    </button>
                ...
                {selectedNotification?.id === notification.id && (
                  <div className="mt-4 p-4 bg-gray-800/30 rounded-lg border border-gray-800/40">
                    <h4 className="font-medium text-white mb-2">Security Analysis</h4>
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-sm text-gray-300">
                        <Lock className="w-4 h-4 text-blue-400" />
                        <span>Sender Domain: </span>
                        <code className="px-2 py-1 bg-gray-800/50 rounded">{notification.sender}</code>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-300">
                        <AlertTriangle className="w-4 h-4 text-blue-400" />
                        <span>Required Action: </span>
                        <span className="text-blue-400">{notification.requiredAction}</span>
                      </div>
                      {notification.documentLink && (
                        <div className="flex items-center gap-2 text-sm text-gray-300">
                          <FileText className="w-4 h-4 text-blue-400" />
                          <span>Linked Document: </span>
                          <code className="px-2 py-1 bg-gray-800/50 rounded">{notification.documentLink}</code>
                        </div>
                      )}
                    </div>
                  </div>
