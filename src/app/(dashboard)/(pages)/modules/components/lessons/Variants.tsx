        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <Building2 className="w-8 h-8 text-blue-400" />
              <div>
                <h1 className="text-xl font-medium text-white">B2B Phishing Simulator</h1>
                <p className="text-sm text-gray-400">Interactive Training Scenario</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="bg-gray-800/30 px-4 py-2 rounded-lg flex items-center gap-2">
                <Clock className="w-5 h-5 text-blue-400" />
                <span className="text-xl font-bold text-white">{score}</span>
              </div>
              <button
                onClick={() => setShowHints(!showHints)}
                className="bg-gray-800/30 p-2 rounded-lg text-blue-400 hover:bg-gray-800/50"
              >
                <Eye className="w-5 h-5" />
              </button>
            </div>
          ...
          {/* Scenario Context */}
          <div className="bg-gray-800/30 p-4 rounded-lg border border-gray-800/40 mb-6">
          ...
          {/* Email Viewer */}
          <div className="bg-gray-800/30 rounded-lg overflow-hidden border border-gray-800/40 mb-6">
            <div className="bg-gray-800/50 p-4">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleElementClick('domain')}
                    className="text-left hover:bg-gray-800/30 p-2 rounded transition-colors"
                  >
                    <div className="font-medium text-white">{currentScenario.sender}</div>
                    <div className="text-sm text-gray-400">To: finance@techcorp.com</div>
                  </button>
                </div>
                <span className="text-sm text-gray-400">{currentScenario.timestamp}</span>
              </div>
              <div className="font-medium text-lg text-white">{currentScenario.title}</div>
            </div>
            
            <div className="p-6 space-y-4">
              <button
                onClick={() => handleElementClick('content')}
                className="text-left w-full whitespace-pre-wrap hover:bg-gray-800/30 p-2 rounded text-gray-300 transition-colors"
              >
                {currentScenario.content}
              </button>
              ...
              {currentScenario.attachments && (
                <div className="border-t border-gray-800/40 pt-4">
                  <button
                    onClick={() => handleElementClick('attachment')}
                    className="flex items-center gap-2 text-blue-400 hover:bg-gray-800/30 p-2 rounded transition-colors"
                  >
                    <FileText className="w-5 h-5" />
                    {currentScenario.attachments[0]}
                  </button>
          ...
          {/* Action Selection */}
          <div className="mb-6">
            <h3 className="font-medium text-white mb-4">Select Appropriate Actions:</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[...currentScenario.correctActions, ...currentScenario.incorrectActions]
                .sort(() => Math.random() - 0.5)
                .map((action, index) => (
                  <button
                    key={index}
                    onClick={() => handleActionSelect(action)}
                    className={`p-4 rounded-lg border-2 text-left flex items-center gap-2 transition-all ${
                      selectedActions.includes(action)
          ...
          {showHints && (
            <div className="bg-blue-500/10 p-4 rounded-lg border border-blue-500/20 mb-6">
              <div className="flex items-center gap-2 mb-3">
                <Eye className="w-5 h-5 text-blue-400" />
                <span className="font-medium text-white">Security Tips</span>
              </div>
              <ul className="space-y-2">
                {currentScenario.suspiciousElements.map(element => (
                  <li key={element.id} className="text-sm text-gray-300">
                    • {element.hint}
                  </li>
                ))}
              </ul>
          ...
          <button
            onClick={checkResults}
            className="w-full bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 border border-blue-500/20 py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition-colors"
          >
            Submit Analysis
            <ArrowRight className="w-5 h-5" />
          </button>
          ...
          {showFeedback && (
            <div className="mt-6">
              <Alert className={score >= 70 
                ...
                <AlertDescription className={score >= 70 ? "text-green-400" : "text-red-400"}>
                  {score >= 70 
                    ? "Great job! You successfully identified the key phishing indicators and selected appropriate actions."
                    : "Some room for improvement. Review the security tips and try again to better identify phishing indicators."}
                </AlertDescription>
